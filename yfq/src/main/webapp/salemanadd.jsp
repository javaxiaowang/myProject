	...
	$(function(){
		 ...
		 $("#submit").click(function(){
		 	...
		 	var name = $("#name").val();
		  	...
		  	var city = $("#city").val();
		  	var saleManType = $("#saleManType").val(); 
		  	if(name!=""&&phone!=""&&oper!=""&&card!=""&&city!=""&&saleManType!=""&&passWord!=""&&confirm!=""){
		  		if(passWord!=confirm){
		  			$("#confirmCheck").html("两次输入密码不一致");
		  			return false;
		  		}
		  		submit(name,phone,oper,card,city,saleManType,passWord);
		  		return true;
		  	}else{
		  		if(name==""){
		 ...
		 function submit(name,phone,oper,card,city,saleManType,passWord){
		 //手机号正则表达式
			...
			if(strPhone==null){
		 	...
		 	$.ajax({
				url:"insertSaleman",
		      	...
		      	async:true,
		      	dataType:"json",
		      	data: {name:name,phone:phone,saleManCard:card,operator:oper,city:city,saleManType:saleManType,passWord:passWord},
		...
		<div class="panel-body" >
			<form action="userInsertAndUpdate" method="post">
				...
				<div style="float:right;margin-right: -20px"><label id="codeCheck" style="line-height: 34px;color: red"></label></div>
				 <br>
				 <br> -->
				<label style="float: left;width: 85px;line-height: 34px">类型</label>
				<select id="saleManType" class="form-control"style="width: 374px" name="saleManType" style="display: block;width: 50%">
					 <option value="3" >客户经理</option>
					 <option value=0 >客户经理管理员</option>
				</select>
				 <br>
