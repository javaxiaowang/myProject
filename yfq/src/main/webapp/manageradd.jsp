	...
	$(function(){
		 ...
		 $("#submit").click(function(){
		 	...
		 	var name = $("#name").val();
		  	...
		  	if(name!=""&&phone!=""&&oper!=""&&card!=""&&city!=""&&passWord!=""&&confirm!=""){
		  		if(passWord!=confirm){
		  			$("#confirmCheck").html("两次输入密码不一致");
		  			return false;
		  		}
		  		submit(name,phone,oper,card,city,passWord);
		  		return true;
		  	}else{
		  		if(name==""){
		 ...
		 function submit(name,phone,oper,card,city,passWord){
		 //手机号正则表达式
				...
				if(phoneStatus===2){
			 	...
			 	$.ajax({
					url:"insertSaleman",
			      	...
			      	async:true,
			      	dataType:"json",
			      	data: {name:name,phone:phone,saleManCard:card,operator:oper,city:city,passWord:passWord},
			      	success:function(data){
			      	if(data===1){
  ...
  <!--表单  -->
	 <div class="panel" style="width: 800px;height:600px;margin-left: 100px;margin-top: 10px">
		<div class="panel-heading">
			<h3 class="panel-title">新增客户管理员</h3>
		</div>
