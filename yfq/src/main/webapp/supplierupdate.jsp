...
<!--获取从上一页面传来的数据 -->
		...
		String sup=new String(request.getParameter("sup").getBytes("ISO-8859-1"),"UTF-8");
		 ...
		 $("#submit").click(function(){
		 	...
		 	var suppliername = $("#suppliername").val();
		  	...
		  	if(suppliername!=""&&contacts!=""&&phone!=""&&email!=""&&city!=""&&address!=""){
		  		submit(id,suppliername,contacts,phone,email,city,address);
		  		return true;
		  	}else{
		  		alert("提交的资料中不允许有空值,请重新提交");
		  	}
		 });
		 function submit(id,suppliername,contacts,phone,email,city,address){
		 	$.ajax({
				url:"supplierInsertAndUpdate",
		      	...
		      	async:true,
		      	dataType:"json",
		      	data: {id:id,supplier:suppliername,contacts:contacts,phone:phone,email:email,city:city,address:address,type:2},
		...
		<div class="panel-body" >
			<form action="userInsertAndUpdate" method="post">
				 <label style="float: left;width: 80px;line-height: 34px">供应商</label>
				  <input id="supid" type="hidden" required="required" class="form-control"  style="display: block;width: 50%">
